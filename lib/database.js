...
class ChatDatabase extends Dexie {
        ...
        try {
            ...
            if (storedPageState) {
                try {
                    ...
                    console.warn('Could not parse pageState, trying chatHistory');
                }
            }
            ...
            if (chatHistory.length === 0 && storedChatHistory) {
                try {
                    ...
                    console.warn('Could not parse chatHistory');
                }
            }

            if (chatHistory.length === 0) {
                console.info('No localStorage data to migrate');
                return { migrated: false, reason: 'no_data' };
            }
            if (existingChats.length > 0 &&  existingChats.length === chatHistory.length) {
                console.info('IndexedDB already has data, skipping migration');
            ...
            console.info(`Successfully migrated ${migratedCount} chats to IndexedDB`);
    ...
    async clearAllData() {
        ...
        console.info('All chat data cleared from IndexedDB');
    }
