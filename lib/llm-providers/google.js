...
export async function* sendMessageStream({ apiKey, model, prompt, history, systemInstruction, temperature }) {
  ...
  try {
    ...
    // Emit provider usage from aggregated streaming response if available
    try {
      const aggregatedForUsage = await streamResult.response;
      const um = aggregatedForUsage?.usageMetadata;
      if (um && !usageEmitted) {
  ...
  if (!yieldedAny && streamResult) {
    try {
      ...
      // Emit provider usage if not yet emitted
      try {
    ...
    try {
      ...
      if (typeof fullText === 'string' && fullText.trim().length > 0) {
        ...
        if (blockReason && String(blockReason).toUpperCase().includes('SAFETY')) {
          throw new Error('Provider safety blocked the response.');
        }
      }
    } catch (e) {
